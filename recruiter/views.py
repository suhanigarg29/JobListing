...
def rec_details(request):
    ...
    return render(request, 'recruiter/details.html', context)
...
def all_jobs(request):
    ...
    return render(request, 'recruiter/job_posts.html', context)
...
def job_candidate_search(request, slug):
    ...
    for applicant in applicants:
        ...
        if (len(common_skills) != 0 and len(common_skills) >= len(job_skills)//2):
            relevant_candidate.append(applicant)
            common.append(len(common_skills))
    objects = zip(relevant_candidate, common)
    ...
    context = {
        ...
        'relevant': len(relevant_candidate),

    }
    return render(request, 'recruiter/job_candidate_search.html', context)
